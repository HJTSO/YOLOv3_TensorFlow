    ...
    for epoch in range(args.total_epoches):
        ...
        if epoch % args.save_epoch == 0 and epoch > 0:
            if loss_total.average <= 2.:
                saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{}_loss_{:.4f}_lr_{:.5g}.ckpt'.format(epoch, int(__global_step), loss_total.average, __lr))
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            if mAP > best_mAP:
                best_mAP = mAP
                saver_best.save(sess, args.save_dir + 'best_model_Epoch_{}_step_{}_mAP_{:.4f}_loss_{:.4f}_lr_{:.7g}.ckpt'.format(
